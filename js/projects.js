...
const PROJECTS = [
  { status: "ongoing", title: "A Study on Trustworthy Performance Index in Reinforcement Learning for Controllers using Customer Feedback", desc: "A methodology study that corrects the errors inherent in customer feedback and removes unnecessary signals, thereby improving the reliability of the Performance Index (PI) model and integrating it into the controller.", keys: "Hyundai · Control · Estimation", tag: "av", img: "/images/funds/hyundai-logo-wide-blue.svg", logo: true },
...
const projectRow = (p) => `
  <article class="prow">
    ${p.img ? `<img class="prow__media${p.logo ? " prow__media--logo" : ""}" src="${p.img}" alt="${p.title}" />` : `<div class="prow__media" data-label="project image"></div>`}
